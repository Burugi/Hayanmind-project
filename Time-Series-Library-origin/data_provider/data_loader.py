...
class Dataset_LongCTR(Dataset):
    """
    Dataset for LongCTR-style binary classification on user behavior sequences.
    Compatible with FuxiCTR data format: parquet files with user_index, item_index, seq_len, label.
    """
    def __init__(self, args, root_path, flag='train', size=None, data_path='train_longctr.parquet',
                 user_info='user_info.parquet', item_info='item_info.parquet', max_len=50, padding='pre'):
        self.args = args
    ...
    def __read_data__(self):
        ...
        user_info_file = os.path.join(self.root_path, 'user_info.parquet')
        item_info_file = os.path.join(self.root_path, 'item_info.parquet')

        df = pd.read_parquet(data_file)
        user_info_df = pd.read_parquet(user_info_file)
        item_info_df = pd.read_parquet(item_info_file)

        self.user_seqs = user_info_df["full_item_seq"].values
        self.item_info = item_info_df.set_index("item_index")

        self.user_indices = df["user_index"].values
        self.item_indices = df["item_index"].values
        self.seq_lens = df["seq_len"].values
        self.labels = df["label"].values

        self.num_samples = len(df)

    def _pad_sequence(self, seq, max_len, padding='pre', value=0):
        """Manual padding to avoid keras_preprocessing dependency."""
        seq = np.array(seq)
        if len(seq) >= max_len:
            if padding == 'pre':
                return seq[-max_len:]
            ...
            if padding == 'pre':
                return np.pad(seq, (pad_width, 0), 'constant', constant_values=value)
            else:
                return np.pad(seq, (0, pad_width), 'constant', constant_values=value)

    def __getitem__(self, index):
        user_idx = self.user_indices[index]
        item_idx = self.item_indices[index]
        seq_len = min(self.seq_lens[index], self.max_len)
        label = self.labels[index]

        user_seq = self.user_seqs[user_idx][:seq_len]

        batch_seq = self._pad_sequence(user_seq, self.max_len, padding=self.padding, value=0)

        full_seq = np.append(batch_seq, item_idx)

        item_features = self.item_info.loc[full_seq]["item_id"].values

        seq_x = torch.from_numpy(item_features[:-1].reshape(-1, 1)).float()
        padding_mask = torch.from_numpy((batch_seq > 0).astype(np.float32))

        return seq_x, torch.tensor(label).float(), padding_mask
