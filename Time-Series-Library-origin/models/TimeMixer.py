...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name in ['classification', 'binary_classification']:
            ...
            self.projection = nn.Linear(
                configs.d_model * configs.seq_len, num_classes)
    ...
    def classification(self, x_enc, x_mark_enc):
        # For classification, use simple embedding without multi-scale decomposition
        B, T, N = x_enc.size()

        # Normalize
        x = self.normalize_layers[0](x_enc, 'norm')

        # embedding
        enc_out = self.enc_embedding(x, None)  # [B,T,C]

        # Output without using pdm_blocks (which requires multi-scale)
        # the output transformer encoder/decoder embeddings don't include non-linearity
        output = self.act(enc_out)
        output = self.dropout(output)
        # zero-out padding embeddings (x_mark_enc is padding_mask)
        if x_mark_enc is not None:
