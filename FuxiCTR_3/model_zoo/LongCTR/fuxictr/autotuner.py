...
def enumerate_params(config_file, exclude_expid=[]):
    ...
    # key checking
    # Allow new parameters for imbalanced data handling and sampling
    allowed_new_params = {
        'use_pos_weight', 'sampling_method', 'target_pos_ratio', 'num_neg_samples',
        'train_sampling', 'train_sampling_ratio', 'eval_negative_sampling_ratio'
    }
    tuner_keys = set(tune_dict.keys())
    base_keys = set(model_dict.keys()).union(set(dataset_dict.keys())).union(allowed_new_params)
    ...
    # First, update model_dict with existing keys
    model_dict = {k: tune_dict[k] if k in tune_dict else [v] for k, v in model_dict.items()}
    # Then, add new parameters from tune_dict that are in allowed_new_params
    for param in allowed_new_params:
        if param in tune_dict and param not in model_dict:
            model_dict[param] = tune_dict[param]
