...
class BaseModel(nn.Module):
    def __init__(self,
        ...
        self._use_pos_weight = kwargs.get("use_pos_weight", False)
        if self._use_pos_weight:
            self.output_activation = nn.Identity()
        self._sigmoid_for_pred = nn.Sigmoid()
    ...
    def compute_loss(self, return_dict, y_true):
        if self._use_pos_weight and self.pos_weight is not None:
            y_pred = return_dict["y_pred"]
            loss = torch.nn.functional.binary_cross_entropy_with_logits(
                y_pred, y_true, pos_weight=self.pos_weight, reduction='mean'
            )
        else:
    ...
    def fit(self, data_generator, epochs=1, validation_data=None,
        ...
        if self._use_pos_weight and self.pos_weight is None:
            self.pos_weight = self.compute_pos_weight(data_generator)
            logging.info("Computed pos_weight: {:.4f}".format(self.pos_weight.item()))

        logging.info("Start training: {} batches/epoch".format(self._steps_per_epoch))
        logging.info("************ Epoch=1 start ************")
        for epoch in range(epochs):
            self._epoch_index = epoch
            if epoch > 0 and hasattr(data_generator, 'resample'):
                data_generator.resample()
    ...
    def eval_step(self):
        ...
        val_logs, _ = self.evaluate(self.valid_gen, metrics=self._monitor.get_metrics())
    ...
    def evaluate(self, data_generator, metrics=None):
        ...
        # Scalability measurement initialization
        import time
        ...
        with torch.no_grad():
            ...
            for batch_data in data_generator:
                ...
                if self._use_pos_weight:
                    pred = self._sigmoid_for_pred(pred)
        ...
        # Scalability measurement completion
        ...
        return val_logs, scalability_metrics
    ...
    def compute_pos_weight(self, data_generator):
        """
        ...
        neg_count = 0
        for batch_data in data_generator:
            labels = self.get_labels(batch_data).cpu().numpy()
            pos_count += (labels == 1).sum()
            neg_count += (labels == 0).sum()

        if pos_count == 0:
            logging.warning("No positive samples found in training data!")
            return torch.tensor(1.0).to(self.device)

        pos_weight = neg_count / pos_count
        logging.info(f"Label distribution - Positive: {pos_count}, Negative: {neg_count}")
